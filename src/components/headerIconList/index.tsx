...
import style from "./HeaderIconList.module.scss";
...
interface IHeaderIconListProps {
  isVisible: boolean;
}

const HeaderIconList: React.FC<IHeaderIconListProps> = ({
  isVisible,
}) => {
  const { productsCart, totalCount } = useAppSelector(
    ...
    >
      ...
      <li className={style.headerIconListItem}>
        <Link to={"/cart"} className={style.cartIcon}>
          ...
          {productsCart.length ? (
            <div
              className={classNames(style.iconCartCount, {
                [style.white]: isVisible ? "white" : "",
              })}
            >
              {totalCount}
            </div>
